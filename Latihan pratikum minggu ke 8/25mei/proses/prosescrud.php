...
class prosesCrud {
 ...
 $row = $this->db->prepare('SELECT * FROM tbl_user WHERE
username=? AND password=md5(?)');
 ...
 // variable $tabel adalah isi dari nama table database yang ingin
ditampilkan
...
?");
 ...
 $sql = "INSERT INTO $tabel (" . implode(", ", $columnNames) .
") VALUES " . implode(", ", $rowsSQL);
 ...
 {
 // sumber kode
 // https://stackoverflow.com/questions/23019219/creatinggeneric-update-function-using-php-mysql
 $setPart = array();
